...
export default function App(){
  ...
  var toggleTheme=function(){var next=theme==="dark"?"light":"dark";setTheme(next);storage.set("br3-theme",next)};
  ...
  var I={background:T.bgInput,border:"1px solid "+T.borderInput,color:T.text,padding:"6px 10px",fontFamily:"'DM Mono',monospace",fontSize:fs(13),width:"100%",outline:"none",boxSizing:"border-box"};
  ...
  var B1={background:T.accent,color:T.accentText,border:"none",padding:"8px 18px",fontFamily:"'DM Mono',monospace",fontSize:fs(12),fontWeight:700,cursor:"pointer",letterSpacing:"0.05em",textTransform:"uppercase"};
  var B2={background:"transparent",color:T.accent,border:"1px solid "+T.accent,padding:"8px 18px",fontFamily:"'DM Mono',monospace",fontSize:fs(12),cursor:"pointer",letterSpacing:"0.05em",textTransform:"uppercase"};

  return (
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'DM Mono','IBM Plex Mono','Courier New',monospace",transition:"background 0.2s,color 0.2s"}}>
      ...
      <header style={{borderBottom:"1px solid "+T.borderLight,padding:"28px 32px",background:T.headerGrad}}>
        <div style={{maxWidth:1020,margin:"0 auto"}}>
          ...
          <h1 style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:fs(28),fontWeight:400,color:T.text,margin:0}}>Back Rent Computation</h1>
          <h2 style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:fs(18),fontWeight:400,color:T.textMuted,margin:"4px 0 0"}}>Federal-Seized & State-Leased Ceded Public Trust Lands - Hawaii</h2>
        ...
        <div style={{maxWidth:1020,margin:"0 auto",display:"flex",gap:0,overflow:"auto"}}>
          {NAV_ITEMS.map(function(n){
            ...
            return <button key={n.id} onClick={function(){scrollTo(n.id)}} style={{
              ...
              fontFamily:"'DM Mono',monospace",fontSize:fs(11),letterSpacing:"0.06em",
              textTransform:"uppercase",whiteSpace:"nowrap",transition:"all 0.15s",
            }}>{n.label}</button>
          })}
          <div style={{flex:1}}/>
          <div style={{display:"flex",alignItems:"center",gap:6,padding:"0 4px"}}>
            <span style={{color:T.accent,fontSize:fs(13),fontWeight:700,fontFamily:"'DM Mono',monospace"}}>{fmt(grandTotal)}</span>
            <span style={{color:T.textFaintest,fontSize:fs(10),marginRight:8}}>{totalAcres.toLocaleString()} ac</span>
            {/* Font size controls */}
            <button onClick={function(){adjustFont(-0.1)}} title="Decrease text size" style={{background:"none",border:"1px solid "+T.borderInput,color:T.textMuted,width:26,height:26,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:11,display:"flex",alignItems:"center",justifyContent:"center",padding:0,opacity:fontScale<=0.8?0.3:1}}>A-</button>
            <button onClick={function(){adjustFont(0.1)}} title="Increase text size" style={{background:"none",border:"1px solid "+T.borderInput,color:T.textMuted,width:26,height:26,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:11,display:"flex",alignItems:"center",justifyContent:"center",padding:0,opacity:fontScale>=1.4?0.3:1}}>A+</button>
            {/* Theme toggle */}
            <button onClick={toggleTheme} title={theme==="dark"?"Switch to light mode":"Switch to dark mode"} style={{background:"none",border:"1px solid "+T.borderInput,color:T.textMuted,width:26,height:26,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",padding:0}}>{theme==="dark"?"\u2600":"\u263D"}</button>
          </div>
        </div>
      </nav>
      ...
      <main style={{maxWidth:1020,margin:"0 auto",padding:"24px 32px 60px"}}>
        ...
        <div ref={function(el){sectionRefs.current["nav-summary"]=el}} id="section-summary" style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:1,background:T.bgOverlay,border:"1px solid "+T.border,marginBottom:24,scrollMarginTop:52}}>
              ...
              {parcelsInCat.map(function(p){
                ...
                return <div key={p.id} style={{background:isSel?T.bgParcelOn:T.bgParcelOff,border:"1px solid "+(isSel?cat.c+"33":T.borderFaint),borderLeft:"2px solid "+(isSel?cat.c:T.borderLight),marginBottom:2,transition:"all 0.1s"}}>
                  <div onClick={function(){toggle(p.id)}} style={{display:"grid",gridTemplateColumns:"28px 1fr 120px",alignItems:"start",padding:"10px 14px",cursor:"pointer",gap:8}}>
                    ...
                    <div style={{minWidth:0}}>
                      <div style={{display:"flex",alignItems:"baseline",gap:8,flexWrap:"wrap"}}>
                        ...
                        <span style={{fontSize:fs(9),padding:"1px 5px",background:cat.c+"18",color:cat.c,border:"1px solid "+cat.c+"33"}}>{catLabelMap[p.category]}</span>
                      </div>
                  ...
                  {isSel&&<div style={{padding:"0 14px 10px 42px"}}>
                    ...
                    <button onClick={function(e){e.stopPropagation();setExpandedParcel(isExpanded?null:p.id)}} style={{background:"none",border:"1px solid "+T.borderLight,color:T.textFaint,padding:"2px 8px",fontSize:fs(9),cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>{isExpanded?"Hide":"Show"} year-by-year</button>
          ...
          <div style={{background:T.bgAlt,border:"1px solid "+T.border,padding:20,color:T.textFaint,fontSize:fs(12),lineHeight:1.7}}>
            ...
            <div style={{marginTop:16,borderTop:"1px solid "+T.borderLight,paddingTop:12}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:4}}>
                ...
                ].map(function(s,i){return s.u?<a key={i} href={s.u} target="_blank" rel="noopener noreferrer" style={{color:T.link,fontSize:fs(11),textDecoration:"none",borderBottom:"1px dotted "+T.link+"44",lineHeight:1.7}}>{s.l}</a>:<span key={i} style={{color:T.link,fontSize:fs(11),lineHeight:1.7}}>{s.l}</span>})}
              </div>
            </div>
          </div>
        </section>
